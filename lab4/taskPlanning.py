...
class PlanningController:
    ...
    def change_traj(self):
        ...
        if self.num_eaten < self.num_food:
            # 还没有全部吃完
            next_food = np.array(
                scene.foods[self.eat_sequence[self.num_eaten]])
            self.planner.find_path(
                self.physice_info.pacman_position(), next_food)
        else:
            # 全部吃完了
            self.scene.run = False
            self.scene.use_time = self.cnt
            print(
                f"Pacman use {self.cnt} steps to eat all the food. Please Exit!")

    def apply_pd_force(self):
        if self.scene.run:
            target_pose = self.planner.get_target(
                self.physice_info.pacman_position(), self.physice_info.pacman_velocity())
        ...
        if self.scene.run:
            ...
            if self.scene.check_eat(self.eat_sequence[self.num_eaten]):
                ...
                if self.cnt > 5000:
                    self.scene.run = False
                    print(
                        f"Failed! Only {self.num_eaten} food has been eaten!")
...
if __name__ == "__main__":
    args = readArgparse()
    if not args.test:
        seq_file = 'data_q3/seq2.txt'  # 自己修改测试
