...
def q1():
    ...
    for i in range(17):
        output = programcall(
            f"{PYTHON_PATH} taskLocalization.py --test --test_idx {i}")
        print('\n'.join(output))
        errors.append(filterq1(output))
        # print(np.exp(-np.mean(errors)))
...
def q2():
    ...
    for i in range(answers.shape[0]):
        output = programcall(
            f"{PYTHON_PATH} taskPDControl.py --test --test_idx {i}")
        print('\n'.join(output))
...
def q3():
    ...
    for i in range(6):
        output = programcall(
            f"{PYTHON_PATH} taskPlanning.py --test --test_idx {i}")
        print('\n'.join(output))
        ...
        if flag is False:
            ...
            task_score = (
                20 + scorer_q3(cnt, stdcnt[i], zerocnt[i], fullcnt[i]) * 0.8)
