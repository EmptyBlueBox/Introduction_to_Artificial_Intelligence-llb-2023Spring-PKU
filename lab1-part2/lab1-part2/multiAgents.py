...
class MCTSAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        EPOCHS = 2500  # 模拟次数 2000
        ITERATION = 15  # 每次模拟的最大深度 20
        THRESHOLD = 10  # 模拟的阈值 10
        NEAREST_K = 1  # 最近的k个节点 1
        index2action = ['North', 'East', 'West', 'South', 'Stop']
        action2index = {'North': 0, 'East': 1,
                        'West': 2, 'South': 3, 'Stop': 4}

        def Selection(node, agent_index):
            if node.game_state.isWin():  # 如果当前节点是胜利节点，直接返回1
                return 1, node, agent_index
            elif node.game_state.isLose():  # 如果当前节点是失败节点，直接返回0
                return 0, node, agent_index
            elif node.is_leaf:  # 如果当前节点是叶子节点，直接返回未定的输赢节点
                return -1, node, agent_index

            nxt_agent_index = (agent_index + 1) % AGENT_NUM  # 下一个agent的index
            ucb_val = -float('inf')  # 最大的ucb值
            ln_t = log(node.denominator)  # 当前节点的模拟次数的对数
            nxt_node = None  # 最大的ucb值对应的节点
            # 如果是pacman就有5个动作（可以Stop），否则就有4个动作
            for child_index in range(5 if agent_index == 0 else 4):
                if node.child_node[child_index] is not None:  # 如果子节点已经expand过
                    ...
                    # 如果子节点已经expand并且已经simulate，那么选择ucb最大的节点
                        ...
                        q = node.child_node[child_index].numerator / \
                            node.child_node[child_index].denominator  # q是子节点的胜率
                        # n是子节点的模拟次数
                        n = node.child_node[child_index].denominator
                        tmp_ucb_val = q+C*sqrt(ln_t/n)  # 这个子节点的ucb值
