...
def get_top_k(U, S, V, k):
    ...
    new_U, new_S, new_V = U[:244, :k], S[:k], V[:k, :207]
...
def matmul_U_S_V(U, S, V):
    ...
    new_mat = np.dot(np.dot(U, np.diag(S)), V)
    return new_mat
...
def task4(U, S, V):
    ...
    for i, k in enumerate(k_list):
        ...
        plt.subplot(2, 2, i+1, title='k={}, eta={:.2f}'.format(k,
                    curve[k]), xticks=[], yticks=[])
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument(
        '--task', type=str, default='task1', help='task name')
    ...
    task_name = args.task
    # 这里的globals()[task_name]相当于调用了task1函数，如果task_name='task2'，则会调用task2函数
    globals()[task_name](U, S, V)
